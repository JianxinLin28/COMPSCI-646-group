...
class OpenAILM():
    ...
    def log_output(self, output):
        my_logger.info("OpenAI Log Output:")
        my_logger.info(output.choices[0].message.content, 1)
        my_logger.info(output.usage, 1)
...
class HFLM():
    ...
    def log_output(self, output):
        my_logger.info("Multi-turn OpenAI Log Output:")
        my_logger.info(output[0].outputs[0].text, 1)
        my_logger.info(f"Number of input tokens: {len(output[0].prompt_token_ids)}", 1)
        my_logger.info(f"Number of output tokens: {len(output[0].outputs[0].token_ids)}", 1)
