...
def load_training_data(folder_path, num_docs=None, subject=None):
    all_data = {}
    my_logger.info(f"Loading data from {folder_path}...")
    ...
    for file_subject, data in all_data.items():
        my_logger.info(f"Subject: {file_subject}, number of examples: {len(data)}", 1)
    return all_data
...
def document_filter(documents, doc_ids, filter_name=None, threshold=1, return_scores=False, num_docs=None, num_sample_pool=None, cache_dir=None, debug=False):
    ...
    if filter_name is None:
        my_logger.info("No filter specified. Using the default length filter.")
        filter_name = 'length'
    ...
    if filter_name == 'fineweb':
        ...
        my_logger.info(f"Using fineweb_edu filter: {filter_name}")
    ...
    if use_cache and os.path.exists(cache_path):
        ...
        scores = [score_dict.get(doc_id, 0) for doc_id in doc_ids]  # jx: ignore missing
    else:
